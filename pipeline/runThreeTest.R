...
K <- 1
##########<<<<<<<<
rtemp <- readRDS("results/resultsThree200.rds")
K <- rtemp$K
L <- rtemp$L
##########<<<<<<<
keep <- 1:K
...
m <- rtemp$m 
cellWeights <- rtemp$lw
globalWeights <- rtemp$gw
rm(rtemp)
print("##################")
nIter <- 200
for(iter in 1:nIter) {
  ...
  if(iter<nIter) {
    ...
    saveRDS(results,"results/resultsTempc3.rds")
  }
}
results <- list()
results$K <- K
...
results$probs <- pcounts
results$counts <- utcounts
saveRDS(results,"results/resultsThree400.rds")

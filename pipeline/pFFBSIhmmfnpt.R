pFFBSIhmmfnpt <- function (i, paths, tcounts, initL, lookup, lpi, K0, phi, Kp, at, alpha0, cSize) {
  set.seed(46)
  library(Matrix)
  N1 <-  ncol(phi) / 2
  tcounts1 <- tcounts[[1]]
  ...
  if(D > 5) {
    ...
    #time2 <- Sys.time()
    trans.list <- getTransgnp(paths[[i]]$grids, tcounts1, tcounts2, tcountsg,tempL, K0, lpi, lookup, Kp, at, cSize)
    trans.path <- trans.list$transMat
    emit.path <- getEmitgt(Kp, paths[[i]]$emit, paths[[i]]$grids, phi, lookup, N1)
    # 3) get slices

resetCountsKc <- function(M, K, lookup, cSize, Lmin, Bmin) {
  ...
  tcounts1 <- lapply(1:M, function(i) lapply(1:length(getNbGridsF(i,cSize, Lmin, Bmin)), function(j) NULL ))
  tcounts2 <- lapply(1:M, function(i) NULL)
  for (i in 1:M) {
      near.i <- getNbGridsF(i, cSize, Lmin, Bmin)[getNbGridsF(i, cSize, Lmin, Bmin) > 0]
  ...
  for ( i in 1:M) {
    if (any(c(1,26,495,520) == 1)) {
      near.i <- getNbGridsF(i, cSize, Lmin, Bmin)
      ti <- which(lookup == i)
      if(sum(ti) != 0) {

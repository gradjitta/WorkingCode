...
source("sampleMeanjnmt.R")
source("sampleVariancejnt.R")
...
paths1 <- readRDS("newData/lpathsbT.rds")
...
for(iter in 1:nIter) {
  ...
  # Update Phi
  print("data for phi")
  data.hmm <- dataForfhmmt(paths1, L, lookup1, N0)
  d <- data.hmm$Lhdp
  data <- data.hmm$data
  ...
  print("get probs..")
  pcounts <- getTransProbsfcp(utcounts, cellWeights, alphaTrans, K)
  print("FFBS..")
  ...
  L <- mclapply(1:length(paths1), pFFBSIhmmfnpt, paths1, pcounts, L, lookup1, cellWeights, Kold, phi, K, alphaTrans, alpha0, mc.cores = 8)
  print("update Counts.. ")
  tcountsi <- resetCountsKc(nCell, K, lookup1)
  utcounts <- updateCountsS(nCell, gridlist, paths1, lookup1, K, L, tcountsi)
  alphacounts <- sapply(1:K, function(j) sum( sapply(1:length(L), function(i) L[[i]][1]) == j))
  ...
  if (iter %% 1 == 0) {
    ...
    saveRDS(results,"results/resultsalpha3g100T1.rds")
  }
...
results$probs <- pcounts
results$counts <- utcounts
saveRDS(results,"results/resultsThree800alpha3g100T1.rds")

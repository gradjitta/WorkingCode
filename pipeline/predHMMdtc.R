predHMMdtc <- function(M, sample, stepsahead, pathid, startpos, startcount, ptrans, cSize, Lmin, Bmin) {
 ...
 alphaTrans <-10
 L <- resinit$L
 trans.list <- getTransgnp(paths1[[idp]]$grids, resinit$probs[[1]], resinit$probs[[2]], 
                           resinit$probs[[3]], L[[idp]], resinit$K-1, resinit$lw, lookup1, resinit$K, alphaTrans, cSize)
...
predNextii<- function(ptrans,phi, alphas, gt,lt, lookup1,K, lw,t,dt, flag, cSize, Lmin, Bmin) {
 ...
 gret1 <- predictDt(dt, ptrans, alphas[[pos]], paths1[[idp]]$grids[pos],resinit$L[[idp]][pos] , lookup1,resinit$K, resinit$lw, pos, resinit$phi, cSize, 100, 80)
 print("Simple model")

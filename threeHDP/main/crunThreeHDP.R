...
nCell <- 520
N0 <- 5
resinit <- readRDS("results/resultsalpha3.rds")
K <- resinit$K
m <- resinit$m
cellWeights <- resinit$lw
globalWeights <- resinit$gw
utcounts <- resinit$counts
iter <- resinit$iter
L <- resinit$L
phi <- resinit$phi
...
print("##################")
nIter <- 1000
for(iter in (resinit$iter + 1):nIter) {
  ...
  L <- mclapply(1:length(paths1), pFFBSIhmmfnp, paths1, pcounts, L, lookup1, cellWeights, Kold, phi, K, alphaTrans, alpha0, mc.cores = 8)
  print("update Counts.. ")
  tcountsi <- resetCountsKc(nCell, K, lookup1)
  utcounts <- updateCountsS(nCell, gridlist, paths1, lookup1, K, L, tcountsi)
  alphacounts <- sapply(1:K, function(j) sum( sapply(1:length(L), function(i) L[[i]][1]) == j))
  ...
  if (iter %% 1 == 0) {
    ...
    saveRDS(results,"results/resultsalpha3.rds")
  }
...
results$probs <- pcounts
results$counts <- utcounts
saveRDS(results,"results/resultsThree400alpha3.rds")

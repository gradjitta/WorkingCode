pFFBSIhmmfin <- function (i, paths, tcounts, initL, sample.run1000, lookup, lpi, K0, phi, Kp = 42) {
  set.seed(46)
  library(Matrix)
  tcounts1 <- tcounts[[1]]
  tcounts2 <- tcounts[[2]]
  ...
  if(D > 5) {
    ...
    #time2 <- Sys.time()
    trans.list <- getTransgc(paths[[i]]$grids, tcounts1, tcounts2, tcountsg,tempL, K0, lpi, lookup)
    trans.path <- trans.list$transMat
    emit.path <- getEmitg(K0, paths[[i]]$emit, paths[[i]]$grids, phi, lookup)
    # 3) get slices
    ...
    # initialize alpha 
    #time3 <- Sys.time()
    alphas <- alphaInitg(paths[[i]],tempL, lookup, phi, K0) 
